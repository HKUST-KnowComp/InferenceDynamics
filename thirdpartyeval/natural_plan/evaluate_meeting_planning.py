...
def validator_from_text(
  ...
  person_to_meet = [key for key, value in processed_constraints.items()]
  locations = [key for key in dist_matrix.keys()]
  for step in plan:
    try:
      if step.startswith("You start"):
        ...
        if destination not in locations:
          raise ValueError("Destination {destination} not in locations".format(destination=destination))
        ...
        if person not in person_to_meet:
          raise ValueError("Person {person} not in person_to_meet".format(person=person))
        met_with[person] = 1
...
def compute_score(num_people_list, constraints_list, dist_matrix_list, responses_list: list[str], golden_plan_list: list[str]):
  acc_per_num_people = [0] * MAX_CONSTRAINTS
  acc_for_all = 0
  output_list = []
  for i in range(len(responses_list)):
    num_people = num_people_list[i]
    start_location, initial_time = constraints_list[i][0]
    constraint = process_constraints(constraints_list[i][1:])
    pred_plan = parse_text_plan(responses_list[i])
    score = validator_from_text(pred_plan, constraint, start_location, initial_time, dist_matrix_list[i])
    golden_score = validator_from_text(golden_plan_list[i], constraint, start_location, initial_time, dist_matrix_list[i])
    output_list.append(float(score == golden_score))
    acc_per_num_people[num_people - 1] += score == golden_score
    acc_for_all += score == golden_score
  return output_list, acc_for_all * 1.0 / len(responses_list)
...
def main(argv: Sequence[str]) -> None:
  ...
  for _, obj in data.items():
    num_people_list.append(obj["num_people"])
    constraints_list.append(obj["constraints"])
    dist_matrix_list.append(obj["dist_matrix"])
    responses_list.append(obj["pred_5shot_pro"])
    golden_plan_list.append(obj["golden_plan"])
  output_list, acc_for_all = compute_score(num_people_list, constraints_list, dist_matrix_list, responses_list, golden_plan_list)
  print(output_list)
  print(acc_for_all)
  # acc_per_num_people = [0] * MAX_CONSTRAINTS
  # acc_for_all = 0
