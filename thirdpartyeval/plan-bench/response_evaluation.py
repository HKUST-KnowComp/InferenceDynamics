...
class ResponseEvaluator:
    def __init__(self, config_file, engine, specified_instances, verbose, ignore_existing=False, input_path=None, model_id=None):
        self.engine = engine
        ...
        self.domain_pddl = f'./thirdpartyeval/plan-bench/instances/{self.data["domain_file"]}'
    ...
    def _set_task_params(self, instance_dir=None):
        ...
        self.instance_folder = f'./thirdpartyeval/plan-bench/instances/{instance_dir}/'
        self.instance = f'./thirdpartyeval/plan-bench/instances/{instance_dir}/{self.data["instances_template"]}'
    ...
    def load_json(self, task_name):
        response_dir = f"{self.input_folder}/"
        output_dir = response_dir.replace("response_dict/", "")
        if not self.ignore_existing and os.path.exists(output_dir+f"{self.model_id}.jsonl"):
            load_dir = output_dir
        else:
            assert os.path.exists(response_dir+f"{self.model_id}.jsonl")
            load_dir = response_dir
        with open(load_dir+f"{self.model_id}.jsonl", 'r') as file:
            structured_output = json.load(file)
        return structured_output
            
    def save_json(self, structured_output, task_name):
        output_dir = f"{self.input_folder.replace('response_dict', '')}/"
        # output_dir = f"results/{self.data['domain_name']}/{self.engine}/"        
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        with open(output_dir+f"{self.model_id}.jsonl", 'w') as file:
    ...
    def evaluate_plan(self, task_name):
        ...
        print(f"The model {self.model_id} has {total_correct} correct plans out of {total_instances} instances")
...
if __name__=="__main__":
    ...
    parser.add_argument("--input_path", type=str, required=True, help="Input folder path")
    ...
    config_file = f'./thirdpartyeval/plan-bench/configs/{config}.yaml'
    response_evaluator = ResponseEvaluator(config_file, engine, specified_instances, verbose, ignore_existing, args.input_path, args.model_id)
    eval_plan_dict = {
